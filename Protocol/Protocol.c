...
uint8_t benday=0;  //定义日
uint8_t benDhour=0;//时
uint8_t benminute=0;//分
uint8_t bensecond=0;//秒

unsigned char time[7],Tmp[20],Flag8563_2402=0,temp;
       ...
       if(ret == 0)
       {
        ...
        switch (Recv_HeadPart->Cmd)
        {
			...
			case Pro_W2D_ErrorPackage_Cmd:
				...
				break;
			case Pro_W2D_TimeAck_Cmd: //获取网络时间的回应 Cmd=0x18
				   Time_AckHandle();//获取服务器时间
			    
				//  PCF8563_read(time);
				//  Ben_AckHandle();//获取本地时间
			// printf("\r\nTime: %d  %d %d:%d\r\n",benday,benDhour,benminute,bensecond);	
		//	printf("\r\nTime:%d-%d %d:%d:%d \r\n",Month,Date,Hour,Min,second);//显示服务器时间				
			break;
...
Report:
    if(Report_Flag == 1)
    {    Pro_UART_SendBuf(getTime,9,1);//向服务器请求获取时间
...
{ 
		Month=UART_HandleStruct.Message_Buf[UART_HandleStruct.Message_Len - 10];//月
		Date=UART_HandleStruct.Message_Buf[UART_HandleStruct.Message_Len - 9];//日
		Hour=UART_HandleStruct.Message_Buf[UART_HandleStruct.Message_Len - 8];//时
		Min=UART_HandleStruct.Message_Buf[UART_HandleStruct.Message_Len - 7];//分
		second=UART_HandleStruct.Message_Buf[UART_HandleStruct.Message_Len - 6];//秒
}

void Ben_AckHandle(void)//zhuan16
{		
		benday=((time[4]>>4)*10+(time[4]&0x0F));  //日
		benDhour=((time[3]>>4)*10+(time[3]&0x0F));  //时
		benminute=((time[2]>>4)*10+(time[2]&0x0F)); //分
		bensecond=((time[1]>>4)*10+(time[1]&0x0F)); //秒
}
void Time_Disposal (void)//转bcd
{
	XIEDate=	  ((Date/10)<<4)|(Date%10);
  XIEHour=	  ((Hour/10)<<4)|(Hour%10);
	XIEMin=	    ((Min/10)<<4)|(Min%10);
  XIEsecond=	((second/10)<<4)|(second%10);
	printf("%d%d%d%d",XIEDate, XIEHour,XIEMin,XIEsecond);
}
